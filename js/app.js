...
const SHOP_ITEMS = [
    ...
    },
    {
        ...
        price: 18, buffKey: 'clarityShards'
    }
];
...
function applySoundState(state) {
    ...
    const onDroneScreen = activeScreen &&
        ['screen-status', 'screen-quests'].includes(activeScreen.id);
    if (onDroneScreen) {
...
function playLevelUp() {
    [330,440,550,660].forEach((n,i) => setTimeout(() => playTone(n,0.18,'sawtooth',0.15), i*90));
}
...
function playRankUp() {
    ...
    try {
        ...
        subG.gain.linearRampToValueAtTime(0.35, now+0.08);
        ...
        shG.gain.linearRampToValueAtTime(0.08, now+0.9);
        shG.gain.exponentialRampToValueAtTime(0.001, now+3.0);
...
// ════════════════════════════════════════════════════════════════
// AMBIENT DRONE — status + quests screens
// Four layers: primary 90Hz · detuned 91.5Hz · noise floor · crackle
// ════════════════════════════════════════════════════════════════
const DRONE_CRACKLE    = true;
const DRONE_FREQ_A     = 90,    DRONE_FREQ_B    = 91.5;
const DRONE_GAIN_A     = 0.13,  DRONE_GAIN_B    = 0.10;
const DRONE_BREATH_RATE = 0.06, DRONE_BREATH_MIN = 0.07, DRONE_BREATH_MAX = 0.17;
const DRONE_BREATH_TICK = 80;
const DRONE_NOISE_GAIN  = 0.015, DRONE_NOISE_FREQ = 900, DRONE_NOISE_Q = 1.8;
const DRONE_CRACKLE_GAIN = 0.035;
const DRONE_CRACKLE_MIN  = 12000, DRONE_CRACKLE_MAX = 22000;
const DRONE_LFO_DEPTH    = 4, DRONE_LFO_RATE = 0.3, DRONE_LFO_TICK = 50;

let droneOscA=null, droneGainA=null, droneOscB=null, droneGainB=null;
let droneNoiseNode=null, droneNoiseGain=null;
let droneBreathTimer=null, droneLfoTimer=null, droneCrackleTimer=null;
let droneBreathPhase=0, droneLfoPhase=0;

function startAmbientDrone() {
    if (!droneEnabled || droneOscA) return;
    try {
        const ctx = getAudioCtx(), now = ctx.currentTime;
        droneOscA = ctx.createOscillator(); droneGainA = ctx.createGain();
        droneOscA.connect(droneGainA); droneGainA.connect(ctx.destination);
        droneOscA.type = 'sine'; droneOscA.frequency.value = DRONE_FREQ_A;
        droneGainA.gain.setValueAtTime(0, now);
        droneGainA.gain.linearRampToValueAtTime(DRONE_GAIN_A, now+3);
        droneOscA.start(now);

        droneOscB = ctx.createOscillator(); droneGainB = ctx.createGain();
        droneOscB.connect(droneGainB); droneGainB.connect(ctx.destination);
        droneOscB.type = 'sine'; droneOscB.frequency.value = DRONE_FREQ_B;
        droneGainB.gain.setValueAtTime(0, now);
        droneGainB.gain.linearRampToValueAtTime(DRONE_GAIN_B, now+3);
        droneOscB.start(now);

        droneNoiseNode = ctx.createBufferSource(); droneNoiseGain = ctx.createGain();
        const nf = ctx.createBiquadFilter();
        nf.type = 'bandpass'; nf.frequency.value = DRONE_NOISE_FREQ; nf.Q.value = DRONE_NOISE_Q;
        droneNoiseNode.buffer = getNoiseBuffer(); droneNoiseNode.loop = true;
        droneNoiseNode.connect(nf); nf.connect(droneNoiseGain); droneNoiseGain.connect(ctx.destination);
        droneNoiseGain.gain.setValueAtTime(0, now);
        droneNoiseGain.gain.linearRampToValueAtTime(DRONE_NOISE_GAIN, now+4);
        ...
        droneBreathTimer = setInterval(() => {
            ...
            const gA  = mid + amp*Math.sin(droneBreathPhase);
            const gB  = (mid + amp*Math.sin(droneBreathPhase + Math.PI/4))*(DRONE_GAIN_B/DRONE_GAIN_A);
            const t   = getAudioCtx().currentTime;
            droneGainA.gain.setTargetAtTime(gA, t, 0.3);
            droneGainB.gain.setTargetAtTime(gB, t, 0.3);
        }, DRONE_BREATH_TICK);
        ...
        droneLfoTimer = setInterval(() => {
            ...
            const dev = (player && player.corrupted) ? DRONE_LFO_DEPTH*Math.sin(droneLfoPhase) : 0;
            const t   = getAudioCtx().currentTime;
            droneOscA.frequency.setTargetAtTime(DRONE_FREQ_A+dev, t, 0.05);
            droneOscB.frequency.setTargetAtTime(DRONE_FREQ_B+dev, t, 0.05);
        }, DRONE_LFO_TICK);
...
function scheduleCrackle() {
    const delay = DRONE_CRACKLE_MIN + Math.random()*(DRONE_CRACKLE_MAX-DRONE_CRACKLE_MIN);
...
function fireCrackle() {
    try {
        ...
        f.type = 'bandpass'; f.frequency.value = 700+Math.random()*600; f.Q.value = 1.2;
        noise.connect(f); f.connect(g); g.connect(ctx.destination);
        g.gain.setValueAtTime(0, now);
        g.gain.linearRampToValueAtTime(DRONE_CRACKLE_GAIN, now+0.008);
...
function stopAmbientDrone() {
    if (droneBreathTimer)  { clearInterval(droneBreathTimer);  droneBreathTimer  = null; }
    if (droneLfoTimer)     { clearInterval(droneLfoTimer);     droneLfoTimer     = null; }
    if (droneCrackleTimer) { clearTimeout(droneCrackleTimer);  droneCrackleTimer = null; }
    droneBreathPhase = 0; droneLfoPhase = 0;
    if (!droneOscA) return;
    try {
        const ctx = getAudioCtx(), now = ctx.currentTime;
        ...
        const [oA, oB, nN] = [droneOscA, droneOscB, droneNoiseNode];
        droneOscA = droneOscB = droneGainA = droneGainB = droneNoiseNode = droneNoiseGain = null;
        setTimeout(() => { try{oA.stop();}catch(e){} try{oB.stop();}catch(e){} try{nN.stop();}catch(e){} }, 1600);
    } catch(e) {}
}

// ════════════════════════════════════════════════════════════════
// MAP AUDIO — world map screen only
// Different character from the status drone:
//   · Higher base tone (220Hz) — brighter, more "active"
//   · Faster breathing (0.12Hz — one pulse every ~8s)
//   · Radar ping instead of crackle: 1800Hz sine, ~60ms, every 6–10s
// ════════════════════════════════════════════════════════════════
const MAP_AUDIO_ENABLE  = true;
const MAP_FREQ_A        = 220,  MAP_FREQ_B       = 222;
const MAP_GAIN_A        = 0.10, MAP_GAIN_B       = 0.07;
const MAP_BREATH_RATE   = 0.12, MAP_BREATH_MIN   = 0.05, MAP_BREATH_MAX  = 0.14;
const MAP_BREATH_TICK   = 80;
...
function startMapAudio() {
    ...
    try {
        ...
        mapGainA.gain.setValueAtTime(0, now);
        mapGainA.gain.linearRampToValueAtTime(MAP_GAIN_A, now+2.5);
        mapOscA.start(now);
        ...
        mapGainB.gain.setValueAtTime(0, now);
        mapGainB.gain.linearRampToValueAtTime(MAP_GAIN_B, now+2.5);
        mapOscB.start(now);
        ...
        mapBreathTimer = setInterval(() => {
            ...
            const mid = (MAP_BREATH_MAX+MAP_BREATH_MIN)/2;
            const amp = (MAP_BREATH_MAX-MAP_BREATH_MIN)/2;
            const gA  = mid + amp*Math.sin(mapBreathPhase);
            const gB  = (mid + amp*Math.sin(mapBreathPhase + Math.PI/3))*(MAP_GAIN_B/MAP_GAIN_A);
            const t   = getAudioCtx().currentTime;
            mapGainA.gain.setTargetAtTime(gA, t, 0.2);
            mapGainB.gain.setTargetAtTime(gB, t, 0.2);
        }, MAP_BREATH_TICK);
...
function fireMapPing() {
    ...
    try {
        ...
        const o = ctx.createOscillator(), g = ctx.createGain();
        o.connect(g); g.connect(ctx.destination);
        o.type = 'sine'; o.frequency.value = MAP_PING_FREQ;
        g.gain.setValueAtTime(0, now);
        g.gain.linearRampToValueAtTime(MAP_PING_GAIN, now+0.005);
...
function stopMapAudio() {
    ...
    try {
        const ctx = getAudioCtx(), now = ctx.currentTime;
        [mapGainA, mapGainB].forEach(g => {
            if (!g) return;
            g.gain.setValueAtTime(g.gain.value, now);
            g.gain.linearRampToValueAtTime(0, now+1.2);
        });
        const [oA, oB] = [mapOscA, mapOscB];
        mapOscA = mapOscB = mapGainA = mapGainB = null;
        setTimeout(() => { try{oA.stop();}catch(e){} try{oB.stop();}catch(e){} }, 1300);
    } catch(e) {}
}

// ════════════════════════════════════════════════════════════════
// WORLD MAP — render
// Read-only. Reads from player.stats, player.corrupted, level, rank.
// Writes nothing back to player state.
// ════════════════════════════════════════════════════════════════

// Zone config — terrain character strings tiled across each zone
const ZONE_CONFIG = {
    intelligence: {
        chars:    '┼ ┼ ┼ · ┼ ┼ + ┼ · ┼ ┼ ┼ + · ┼ ┼ ',
        lore:     'SIGNAL GRID'
...
// Fog of war tier thresholds
function zoneTier(statValue) {
    if (statValue <  11) return 0;   // floor — uncharted
    if (statValue <  30) return 1;   // dim — outline only
    if (statValue <  60) return 2;   // partial — terrain appears
    return 3;                        // clear — fully revealed
}
...
// Hub unlocks when ≥3 zones are above the floor (stat > 10)
...
function renderMap() {
    ...
    const corrupt = !!player.corrupted;

    // Coords line in header
    const totalXP = earnedXP(stats);
    document.getElementById('map-coords').textContent =
        'OPR: ' + player.name + '  ·  LV.' + level + '  ·  ' + rank + '-RANK';

    // Corrupted map state
    const viewport = document.getElementById('map-viewport');
    viewport.classList.toggle('map--corrupted', corrupt);

    // Status bar
    document.getElementById('map-scan-status').textContent = corrupt ? 'CORRUPTED' : 'ACTIVE';

    // Populate terrain text once (idempotent — only fills if empty)
    STAT_NAMES.forEach(stat => {
        const terrainEl = document.getElementById('terrain-' + stat);
        if (terrainEl && !terrainEl.textContent.trim()) {
            // Repeat chars to fill the tile
            terrainEl.textContent = ZONE_CONFIG[stat].chars.repeat(12);
        }
    });

    // Apply tier classes and update values for each zone
    STAT_NAMES.forEach(stat => {
        ...
        // Remove existing tier classes, apply current one
        zoneEl.classList.remove('tier-0','tier-1','tier-2','tier-3');
        zoneEl.classList.add('tier-' + tier);

        // Value display — show number once tier 1+, masked at tier 0
    ...
    const isUnlocked = hubUnlocked(stats);
    hubEl.classList.toggle('hub-locked',   !isUnlocked);
    hubEl.classList.toggle('hub-unlocked',  isUnlocked);
    ...
    // Hub pin — pulse faster when corrupted
...
async function init() {
    ...
    document.getElementById('settings-btn').addEventListener('click', () => {
        playUIClick(); openSettings();
    });
    document.getElementById('shop-btn').addEventListener('click', () => {
        playUIClick(); openShop();
    });
    ...
    document.getElementById('map-header-back').addEventListener('click', () => {
        navTo('screen-status');
    });
...
function runAwakenSequence(name) {
    ...
    function next() {
        if (idx >= bl.length) {
            setTimeout(() => typeText(nameEl, name, 80, () =>
                setTimeout(() => typeText(statusEl, '[ AWAKENING... ]', 60, () =>
                    setTimeout(() => { overlay.classList.add('hidden'); createPlayer(name); }, 800)), 400)), 300);
            return;
        }
...
function createPlayer(name) {
    ...
    player = { name, stats, completedToday:[], lastQuestDate:today(), consecutiveDays:1,
        momentum:1.0, lastActiveDate:today(), hp:maxHp, maxHp, corrupted:false, gold:0, buffs:defaultBuffs() };
    savePlayer();
    dailyQuests = getDailyQuests(allQuests, calculateLevel(), effectiveGear());
...
function effectiveGear() {
    return (player && player.buffs && buffActive(player.buffs.sprintScroll))
        ? Math.min(3, currentGear+1) : currentGear;
}
...
function checkDailyReset() {
    ...
    if (diffDays === 1) {
        const covered = new Set((player.completedToday||[]).map(id => {
            const q = allQuests.find(q => q.id===id); return q ? q.stat : null;
        }).filter(Boolean));
        if (covered.size >= 5) player.hp = Math.min(player.maxHp, (player.hp||player.maxHp)+20);
    } else {
        const dmg = diffDays===2?10 : diffDays===3?20 : 35;
        player.hp = Math.max(0, (player.hp||player.maxHp)-dmg);
    }
...
function showLog(msg, variant) {
    const c = document.getElementById('system-log');
    if (!c) return;
    ...
    setTimeout(() => {
        el.classList.add('log-entry--fading');
        setTimeout(() => el.remove(), LOG_FADE);
    }, LOG_LINGER);
}
...
function renderShop() {
    ...
    if (buffActive(buffs.restSigil)) {
        const e=new Date(buffs.restSigil);
        al.push('🌑 REST SIGIL — Momentum protected until '+String(e.getHours()).padStart(2,'0')+':'+String(e.getMinutes()).padStart(2,'0'));
    }
    if (buffs.clarityShards>0) al.push('📝 CLARITY SHARDS — +5 XP on next '+buffs.clarityShards+' directive(s)');
    if (al.length) {
        buffsEl.innerHTML='<div class="active-buffs-title">[ ACTIVE EFFECTS ]</div>'+al.map(l=>'<div class="active-buff-line">'+l+'</div>').join('');
...
function consumeItem(itemId) {
    ...
    switch(itemId) {
        ...
        case 'vitalityTonic': {
            const mh=player.maxHp||calcMaxHp(calculateLevel());
            player.hp=Math.min(mh,(player.hp||mh)+20);
            const pct=Math.round((player.hp/mh)*100);
            const hEl=document.getElementById('hp-bar'), hV=document.getElementById('hp-value');
            if(hEl) hEl.style.width=pct+'%'; if(hV) hV.textContent=player.hp+' / '+mh;
            showLog('[VITALITY_TONIC: +20 HP]'); break;
        }
        case 'sprintScroll':
            player.buffs.sprintScroll=endOfDayISO();
            dailyQuests=getDailyQuests(allQuests,calculateLevel(),effectiveGear());
            showLog('[GEAR_SHIFT: GEAR_'+effectiveGear()+'_ENGAGED]'); break;
        case 'restSigil':
            player.buffs.restSigil=in24hISO();
            showLog('[REST_SIGIL: MOMENTUM_PROTECTED_24H]'); break;
        case 'clarityShards':
            player.buffs.clarityShards=(player.buffs.clarityShards||0)+3; break;
    }
...
function completeQuest(id, stat, baseXP) {
    ...
    let amt=parseFloat((baseXP*momentum).toFixed(1));
    const wasCorrupted=player.corrupted;
    if (wasCorrupted) amt=parseFloat((amt/2).toFixed(1));
    if (buffActive(player.buffs.focusDraught) && (stat==='intelligence'||stat==='endurance'))
        amt=parseFloat((amt*2).toFixed(1));
    ...
    showFloatingXP(id, amt, isCrit);
    if (!isCrit) playQuestComplete();
    ...
    updateStatusScreen();
    if (newRank!==prevRank) {
        setTimeout(() => { showLog('[RECLASSIFIED: '+newRank+'-RANK CONFIRMED]','accent'); showRankUpOverlay(newRank,newLevel); }, 600);
    } else if (newLevel>prevLevel) {
        setTimeout(() => { showLog('[THRESHOLD: LEVEL '+newLevel+' REACHED]','accent'); showLevelUpOverlay(newLevel); }, 600);
    }
}

function calculateLevel() { return levelFromXP(Math.max(0,earnedXP(player.stats))); }
function calculateLuck() {
    return parseFloat((STAT_NAMES.reduce((s,n)=>s+(player.stats[n]||STAT_FLOOR),0)/STAT_NAMES.length).toFixed(1));
}
...
function updateStatusScreen(animate) {
    ...
    const hp=player.hp??player.maxHp, maxHp=player.maxHp??calcMaxHp(level);
    ...
    STAT_NAMES.forEach(stat => {
        const val=player.stats[stat]||STAT_FLOOR, dv=Math.floor(val);
        const bp=Math.min(100,((val-STAT_FLOOR)/90)*100);
        if (animate) {
            animateNumber('val-'+stat,0,dv,600);
            setTimeout(()=>document.getElementById('bar-'+stat).style.width=bp+'%',100);
        } else {
            document.getElementById('val-'+stat).textContent=dv;
            document.getElementById('bar-'+stat).style.width=bp+'%';
        }
    });
...
function showStatusScreenWithAnimation() {
    ...
    setTimeout(() => {
        ...
        if (calculateLevel()>1) setTimeout(()=>showLevelUpOverlay(calculateLevel()),1500);
    }, 200);
}

function animateNumber(elId, from, to, dur) {
    const el=document.getElementById(elId); if(!el) return;
    const steps=30, step=(to-from)/steps, delay=dur/steps;
    let cur=from, cnt=0;
...
function showFloatingXP(questId, amount, isCritical) {
    ...
    const top=rect.top+window.scrollY, left=rect.left+rect.width/2-20;
    if (isCritical) {
        const c=document.createElement('div'); c.className='float-xp float-critical';
        c.textContent='[ CRITICAL ]'; c.style.left=left+'px'; c.style.top=(top-24)+'px';
        document.body.appendChild(c); setTimeout(()=>c.remove(),1100);
    }
    const l=document.createElement('div'); l.className='float-xp'; l.textContent='+'+amount+' XP';
    l.style.left=left+'px'; l.style.top=top+'px';
    document.body.appendChild(l); setTimeout(()=>l.remove(),1000);
}
...
function shareCard({headline,bigText,titleText,subText,accentColor}) {
    ...
    ctx.fillRect(0,0,W,5);ctx.fillRect(0,H-5,W,5);ctx.fillRect(0,0,5,H);ctx.fillRect(W-5,0,5,H);
...
function canvasWrapText(ctx,text,x,y,maxWidth,lh){
    ...
    for(let n=0;n<words.length;n++){
        ...
        if(ctx.measureText(test).width>maxWidth&&n>0){ctx.fillText(line.trim(),x,y);line=words[n]+' ';y+=lh;}
        else line=test;
    }
...
function downloadCanvas(canvas){
    const a=document.createElement('a'); a.download='levelup-moment.png'; a.href=canvas.toDataURL('image/png'); a.click();
}
...
function showScreen(id) {
    ...
    const statusOrQuests = ['screen-status', 'screen-quests'];
    const wasStatus      = prevId && statusOrQuests.includes(prevId);
    const isStatus       = statusOrQuests.includes(id);
    const wasMap         = prevId === 'screen-map';
    const isMap          = id    === 'screen-map';

    // ── Audio transitions ────────────────────────────────────
    if (isStatus && !wasStatus) {
        // Arriving at status/quests from somewhere else
        if (wasMap)  stopMapAudio();
        if (droneEnabled && !droneOscA) startAmbientDrone();
    }
    if (!isStatus && wasStatus) {
        // Leaving status/quests
        if (!isMap) stopAmbientDrone();
        else         stopAmbientDrone();  // map audio starts separately below
    }
    if (isMap) {
        // Always stop status drone when entering map
        stopAmbientDrone();
        if (droneEnabled) startMapAudio();
        renderMap();
    }
    if (!isMap && wasMap) {
        stopMapAudio();
    ...
    if (id === 'screen-quests') renderQuests(dailyQuests, player.completedToday, player.momentum||1.0);
}
